...
func getClient() pb.ImageSvcClient {
	...
	defer cancel()
	conn, err := grpc.DialContext(ctx, tAddr, grpc.WithInsecure())
	if err != nil {
		logger().Fatalw("error in dial", "err", err)
	}
...
func TestImage(t *testing.T) {
	...
	res, err := getClient().Store(ctx, in)
...
func TestFetch(t *testing.T) {
	uri := "http://thirdwx.qlogo.cn/mmopen/vi_32/Q0j4TwGTfTIEGuqUV4YvKAqxHiaFiafK6uCzrn4Z4LlQac6jAWDqtOaJ06w3paicrtKkeQTtiaVQwxp50tkRIVBMjg/0"
	in := &pb.FetchInput{
		ApiKey: apiKey,
		Uri:    uri,
		Roof:   "demo",
	...
	ctx := context.Background()
	res, err := getClient().Fetch(ctx, in)
	assert.NoError(t, err)
	assert.NotNil(t, res)
	t.Logf("res %v", res)
}

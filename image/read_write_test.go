...
func TestJpegReadWrite(t *testing.T) {
	...
	if ia.Width != jpeg_width {
		t.Fatalf("unexpected width:\n+ %v\n- %v", ia.Width, jpeg_width)
	}
	if ia.Height != jpeg_height {
		t.Fatalf("unexpected height:\n+ %v\n- %v", ia.Width, jpeg_height)
	}
	if ia.Quality != jpeg_quality {
		t.Fatalf("unexpected quality:\n+ %v\n- %v", ia.Width, jpeg_quality)
	}
	// var dir string
	// dir, err = os.Getwd()
	...
	wrote_len := out.Len()
	t.Logf("wrote %d bytes", wrote_len)
...
const (
	jpeg_width   = iimg.Dimension(124)
	jpeg_height  = iimg.Dimension(144)
	jpeg_quality = iimg.Quality(88)
	jpeg_size    = iimg.Size(5642)
)
